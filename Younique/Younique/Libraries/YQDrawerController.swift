...
@objc protocol YQDrawerControllerDelegate {
    @objc optional func drawerController(_ drawerController: YQDrawerController, willChangeState state: YQDrawerController.DrawerState)
    @objc optional func drawerController(_ drawerController: YQDrawerController, didChangeState state: YQDrawerController.DrawerState)
}

class YQDrawerController: UIViewController, UIGestureRecognizerDelegate {
    ...
    public private(set) lazy var screenEdgePanGesture: UIScreenEdgePanGestureRecognizer = {
        let gesture = UIScreenEdgePanGestureRecognizer(
            target: self,
            action: #selector(YQDrawerController.handlePanGesture(_:))
        )
    ...
    public private(set) lazy var containerViewTapGesture: UITapGestureRecognizer = {
        let gesture = UITapGestureRecognizer(
            target: self,
            action: #selector(YQDrawerController.didtapContainerView(_:))
        )
        gesture.delegate = self
        return gesture
    }()
    
    public weak var delegate: YQDrawerControllerDelegate?
    ...
    @IBInspectable public var drawerWidth: CGFloat = 120 {
        didSet {
            _drawerWidthConstraint?.constant = drawerWidth
    ...
    override open func viewDidLoad() {
        ...
        self.drawerWidth = UIScreen.main.bounds.width/2
...
class YQEmbedDrawerControllerSegue: UIStoryboardSegue {
    
    final override public func perform() {
        if let sourceViewController = source as? YQDrawerController {
            sourceViewController.drawerViewController = destination
        } else {
            assertionFailure("SourceViewController must be YQDrawerController!")
        }
    }
    
}

class YQEmbedMainControllerSegue: UIStoryboardSegue {
